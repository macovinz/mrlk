...
const SLIDES: Slide[] = [
  {
    ...
    video:
      "/src/assets/blob_gradient_with_alpha_3_VP9.webm",
    portraitSide: "right",
  },
  {
    ...
    image: "/src/assets/silhouettes/C:\Sites\mrlk\frontend\src\assets\silhouettes\sillouette3.webp",
    video:
      "/src/assets/blob_gradient_with_alpha_3_VP9.webm",
    portraitSide: "left",
  },
...
export default function GentleScroller() {
  ...
  useEffect(() => {
    ...
    // build triggers after images are ready to avoid miscalcs
    const ready = () => {
      const sections = gsap.utils.toArray<HTMLElement>(root.current!.querySelectorAll(".vh-section"));
      ...
      // one trigger per section — no pin, natural scroll, buttery with Lenis
    ...
    if (pending === 0) ready();
      ...
      imgs.forEach((im) => { im.addEventListener("load", done, { once: true }); im.addEventListener("error", done, { once: true }); });
    }
  ...
  return (
    <div ref={root} className="relative w-full">
      {/* FIXED BACKGROUND VIDEOS (stay behind sections while scrolling) */}
      ...
      {/* NATURAL FLOW: one full-viewport section per slide */}
      {SLIDES.map((s, i) => {
            ...
            <div className="relative mx-auto flex min-h-[100vh] w-full max-w-7xl items-center px-6">
              ...
              >
                ...
                <div className={`flex flex-col justify-center ${side === "left" ? "md:[direction:ltr]" : ""}`}>
                  ...
                  <p className="vh-desc mt-4 max-w-xl font-body text-lg sm:text-xl text-white/90 opacity-0">{s.desc}</p>
                </div>
              </div>
            </div>
          </section>
